...
#ifndef XARMLIB_TARGETS_LPC81X_SYSCON_CLOCK_HPP
#define XARMLIB_TARGETS_LPC81X_SYSCON_CLOCK_HPP
...
public:
    ...
    static int32_t get_system_pll_in_frequency()
    {
        ...
        {
            case SystemPllSource::irc:         return System::get_irc_frequency();   break; // 12 MHz internal RC oscillator
            case SystemPllSource::sys_osc_clk: return System::get_xtal_frequency();  break; // System oscillator (crystal)
            case SystemPllSource::clk_in:      return System::get_clkin_frequency(); break; // CLK_IN (clock input pin)
    ...
    static int32_t get_main_clock_frequency()
    {
        ...
        {
            case MainClockSource::irc:             return System::get_irc_frequency();    break;
    ...
    static int32_t get_watchdog_osc_frequency()
    {
        ...
        return k_watchdog_osc_frequency[frequency] / ((div + 1) << 1);
    }
    ...
    static constexpr WatchdogConfig get_watchdog_osc_config()
    {
        ...
        for(std::size_t idx = 1; idx < k_watchdog_osc_frequency.size(); ++idx)
        {
            for(uint8_t freq_div = 64; freq_div >= 2; --freq_div)
            {
                const uint64_t freq = k_watchdog_osc_frequency[idx] / freq_div / 4;

                const uint32_t timer_count = static_cast<uint32_t>(freq * duration_us / 1000000);

                if(timer_count >= 0xFF && timer_count <= 0xFFFFFF)
                {
                    return WatchdogConfig{static_cast<WatchdogFrequency>(idx), freq_div, timer_count};
                }
            }
        }
    }
...
private:
    ...
    static constexpr int64_t wdt_min_timeout_us()
    {
        ...
        const int32_t max_freq = k_watchdog_osc_frequency.back() / min_freq_div / 4;
    ...
    static constexpr int64_t wdt_max_timeout_us()
    {
        ...
        const int32_t min_freq = k_watchdog_osc_frequency[1] / max_freq_div / 4;
    ...
    static constexpr std::array<int32_t, 16> k_watchdog_osc_frequency
    {
        0,                      // OSC_ILLEGAL
        600000,                 // OSC_0_60
        1050000,                // OSC_1_05
...
#endif  // XARMLIB_TARGETS_LPC81X_SYSCON_CLOCK_HPP
